...
DRAWIO_OUT = FIG_DIR / "fig_architecture_wide.drawio"
PNG_OUT = FIG_DIR / "fig_architecture_wide.png"
...
def phase_block_style(color_key: str) -> str:
    ...
    return (
        ...
        f"strokeWidth=2;fontSize=10;dashed=1;dashPattern=8 4;"
    )
...
def step_box_style(color_key: str, highlight: bool = False) -> str:
    """步驟方格:白底 + 彩色邊框(在淡色容器上才有對比);highlight=True 用 sky 邊框突顯 (延伸機制)。"""
...
def kb_collection_style(color_key: str) -> str:
    ...
    return (
        ...
        f"fontStyle=1;fontColor={c['border']};verticalAlign=top;spacingTop=24;"
    )
...
def main() -> int:
    ...
    # 圖內不放標題(簡報頁自帶 kicker「02 · 系統整體架構(圖 3.1)」);★ 之意義見底部圖例
    ...
    chip_labels = [
        ("Phase I", "輸入與預處理", "pink"),
        ("Phase II", "RAG 核心檢索", "lavender"),
        ("Phase III", "CoT 推理與輸出", "mint"),
        ("Phase IV", "LLM-as-Judge 評估", "coral"),
    ]
    ...
    # ── Phase II: 5 → 8 ──
    p2_steps = [
        ("5. 向量化 Embedding", "arctic-embed-l-v2.0 · 1024 維"),
        ("6. Top-K 檢索", "ChromaDB cosine · K=3"),
        ("★ 7. 提取 Context", "雙索引交叉佐證 · Triangulator §5.2", True),
    ...
    kb_box_h = 190
    cb.add_vertex(
        f"<b>知識庫 Knowledge Base</b>　ChromaDB · cosine · Top-K = 3",
        kb_box_x,
        kb_y,
        kb_box_w,
        40,
    ...
    laws_id = cb.add_vertex(
        f"<b>LAWS collection</b><br><span style='font-size:9.5px'>全國法規資料庫<br>民法／民訴／消保法<br>2,429 條文片段</span>",
        kb_box_x + 60,
        kb_y + 55,
        380,
        145,
    ...
    judg_id = cb.add_vertex(
        f"<b>JUDGEMENTS collection</b><br><span style='font-size:9.5px'>司法院 OpenData<br>114/5–115/4 · 16,946 案<br>13,897 索引 / 3,049 留出</span>",
        kb_box_x + 560,
        kb_y + 55,
        380,
        145,
