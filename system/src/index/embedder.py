...
def _model() -> SentenceTransformer:
    ...
    # device:預設交由 sentence-transformers 自動偵測;可用 EMBED_DEVICE 覆寫。
    #   於 16GB 統一記憶體之 Apple Silicon,MPS 對 arctic-embed-l 大批次易 OOM,
    #   建索引時可設 EMBED_DEVICE=cpu 以穩定完成(較慢但不爆記憶體)。
    device = os.getenv("EMBED_DEVICE") or None
    return SentenceTransformer(EMBEDDING_MODEL, trust_remote_code=True, device=device)
