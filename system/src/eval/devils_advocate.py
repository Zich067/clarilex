...
對應論文「延伸加修」§5.4。Devil's Advocate 對抗審稿 agent — 預設「找碴」、
唯有提出 ≥ 4/5 之強證據才願意讓步。
...
def challenge(
    ...
    # robustness 依論文定義以三輪 score 之公式計算 (1/15)Σscore，clamp 至 [0.2, 1.0]，
    # 使指標可重現、與論文式一致；LLM 自報之 overall_robustness 僅作為無 score 時之退路。
    _scores = [r.get("score") for r in rounds if isinstance(r.get("score"), (int, float))]
    if _scores:
        overall_robustness = max(0.2, min(1.0, round(sum(_scores) / 15, 4)))
    else:
