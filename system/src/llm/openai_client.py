...
def _chat_json_anthropic(
    ...
    """以原生 anthropic SDK 呼叫 Claude(model 以 "claude" 開頭時)。

    Anthropic API 與 OpenAI 差異:system 為獨立參數(非 message)、無 response_format,
    故靠 prompt 指示輸出純 JSON,再交由 parse_json 擷取 {...}。
    """
    if not ANTHROPIC_API_KEY:
        if allow_mock_fallback:
            log.warning("ANTHROPIC_API_KEY 未設定 — 使用 mock 回應")
            return _mock_response()
        raise RuntimeError("ANTHROPIC_API_KEY 未設定")

    import anthropic  # 延遲載入,未用 Claude 時不需安裝此套件
    ...
    for attempt in range(3):
        try:
            # 新一代 Claude(opus-4-x 等)已棄用 temperature 參數,故不傳(採預設);
            # 舊模型不傳亦安全。DA 之對抗性由 prompt 之非對稱立場決定,不倚賴溫度。
            ...
            return LLMResponse(content=text or "{}", model=model, source="live")
            ...
            break
    if allow_mock_fallback:
        log.warning(f"Anthropic 呼叫失敗:{last_err} — 使用 mock 回應")
        return _mock_response()
    raise last_err if last_err else RuntimeError("Anthropic 呼叫失敗")
...
def chat_json(
    ...
    if model.lower().startswith("claude"):
        return _chat_json_anthropic(messages, model, temperature, timeout, allow_mock_fallback)
